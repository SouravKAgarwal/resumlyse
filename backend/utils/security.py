...
async def validate_file_security(file: UploadFile) -> str:
    ...
    if ext == "pdf":
        if not header.startswith(b"%PDF"):
            ...
            raise HTTPException(status_code=400, detail="Invalid file signature. File does not appear to be a valid PDF.")
    elif ext == "docx":
        if not header.startswith(b"PK\x03\x04"):
            logger.warning(f"File {filename} has invalid magic bytes for DOCX")
            raise HTTPException(status_code=400, detail="Invalid file signature. File does not appear to be a valid DOCX.")
    elif ext == "rtf":
        if not header.startswith(b"{\\rtf1"):
            logger.warning(f"File {filename} has invalid magic bytes for RTF")
            raise HTTPException(status_code=400, detail="Invalid file signature. File does not appear to be a valid RTF.")
    elif ext == "txt":
        # For txt, we can just verify it is valid unicode/ascii without null bytes
        if b'\x00' in header:
            logger.warning(f"File {filename} contains null bytes, treating as invalid TXT")
            raise HTTPException(status_code=400, detail="Invalid file signature. TXT file contains binary data.")
