...
class ExportService:
    ...
    def generate_pdf(analysis: AnalysisResult, filename: str = "Resume") -> bytes:
        ...
        elements.append(
            Paragraph(
                f"{filename} — {datetime.now().strftime('%B %d, %Y')}", subtitle_style
            )
        )
        ...
        score_color = (
            colors.green
            if analysis.overall_score >= 76
            else colors.orange
            if analysis.overall_score >= 51
            else colors.red
        )
        elements.append(
            Paragraph(
                ...
                ParagraphStyle(
                    "ScoreStyle", parent=styles["Normal"], alignment=TA_CENTER
                ),
            )
        )
        ...
        for cat in analysis.category_scores:
            data.append(
                [
                    ...
                    Paragraph(
                        cat.feedback,
                        ParagraphStyle("CellText", parent=styles["Normal"], fontSize=8),
                    ),
                ]
            )
        ...
        st = Table(
            section_data, colWidths=[1.5 * inch, 0.7 * inch, 0.7 * inch, 3.6 * inch]
        )
        st.setStyle(
            TableStyle(
                [
        ...
        for cat in analysis.category_scores:
            if cat.suggestions:
                elements.append(
                    Paragraph(f"<i><u>{cat.category}</u></i>", styles["Normal"])
                )
